...
import { parseAddressKey, type BinderAddress, type BinderSlot } from "../../../models/binderLayout.ts";

/**
 * Dragging a card from one pocket to another.
 *
 * Pointer events, NOT HTML5 drag-and-drop: `dragstart` never fires on touch,
 * and the phone is a real target here. One implementation covers mouse, pen and
 * finger; the alternative is two, and the touch one would be this file anyway.
 *
 * Drop targets are found with `elementFromPoint` against `[data-pocket]` rather
 * than with a handler per pocket. A 12-pocket spread is 24 targets that would
 * each have to track enter and leave and reconcile against a drag that can be
 * cancelled mid-flight; hit-testing once per move is both less code and less
 * state.
 *
 * THREE THINGS BREAK THIS SILENTLY. Each of them was found here the hard way,
 * each produced no error anywhere, and each is handled below with the comment
 * that explains it:
 *
 *   1. Handlers reading React state instead of a ref — a flick completes inside
 *      one frame, so the whole gesture runs against `drag === null`.
 *   2. The browser's own image drag, which wins and stops pointermove.
 *   3. `touch-action` applied after the browser has already claimed the gesture
 *      as a pan, by which point it has taken the pointer away.
 *
 * This is a v2 copy of `src/web/binders/useBinderDrag.ts`, not an import: that
 * file is v1 and stays untouched, and a v2 fix here must not have to be made
 * over there. The two are identical today; the duplication is reported.
 */
...
export type DragSource =
  ...
  /**
   * A card picked up from the picker, which is not IN the binder yet.
   *
...
export interface DragState {
  source: DragSource;
  /** What is being carried, for the ghost and for the drop. */
  slot: BinderSlot;
...
export function useBinderDrag(onDrop: (source: DragSource, slot: BinderSlot, to: BinderAddress) => void) {
   ...
   * In the deps that would tear down and re-attach five document listeners
   ...
   * against `drag === null`, and the drop is silently dropped.
   */
  const dragRef = useRef<DragState | null>(null);

  const clearPending = useCallback(() => {
    const timer = pending.current?.timer;
  ...
  const onPointerDown = useCallback(
    (event: ReactPointerEvent, source: DragSource, slot: BinderSlot) => {
  ...
  /**
   * Document-level listeners, live for the lifetime of the screen.
   *
   * On the document rather than on the element, because a drag routinely leaves
  ...
  useEffect(() => {
     ...
     * finger has not moved, so there is no scroll in progress to interrupt.
     */
     ...
     *
     * Every pocket holds an `<img>`, and an image is `draggable` by default:
     * press on one with a mouse, move, and the browser starts a NATIVE drag —
     * it puts up a translucent copy, stops sending pointermove, and the pocket
     * drag dies on its first frame with no error anywhere.
     *
     * Cancelled here rather than by putting `draggable={false}` on each image,
     * because the images come from three places (a pocket, the cover, the
     * picker) and a fourth would only have to remember.
     */
   ...
   * must apply only while a drag is running — a pocket that permanently refused
   * touch-action could not be scrolled past on a phone.
   */
  useEffect(() => {
    if (!drag) return;
